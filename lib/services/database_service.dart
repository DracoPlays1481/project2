...
class DatabaseService {
  ...
  Future<void> saveUserProfile(UserProfile profile) async {
    try {
      // Use set instead of update to ensure we write the entire profile
      await _database.child('profiles/${profile.uid}').set(profile.toJson());
      print('Profile saved successfully for user: ${profile.uid}');
    } catch (e) {
  ...
  Future<UserProfile?> getUserProfile(String uid) async {
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        print('Profile found for user: $uid');
        ...
        try {
          ...
          print('Successfully parsed profile: ${profile.name}, Weight: ${profile.weight}, Height: ${profile.height}, Age: ${profile.age}');
          return profile;
        } catch (parseError) {
          print('Error parsing profile data: $parseError');
          // Create a default profile with the user ID but preserve any valid data
          final defaultProfile = UserProfile(
            uid: uid,
            name: data['name'] as String? ?? '',
            age: data['age'] is int ? data['age'] : 0,
            weight: data['weight'] is num ? (data['weight'] as num).toDouble() : 0.0,
            height: data['height'] is num ? (data['height'] as num).toDouble() : 0.0,
            gender: data['gender'] as String? ?? 'male',
          );

          // Save the corrected profile to the database
          await saveUserProfile(defaultProfile);
          print('Created and saved corrected profile for user: $uid');
        ...
        await saveUserProfile(defaultProfile);
        print('Created and saved default profile for user: $uid');

        return defaultProfile;
      }
    } catch (e) {
      print('Error getting user profile: $e');
      // Return a basic profile instead of null to prevent crashes
      return UserProfile(
        uid: uid,
        name: '',
        age: 25,
